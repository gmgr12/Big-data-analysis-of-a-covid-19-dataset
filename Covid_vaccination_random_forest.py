...
# Convert 'date' column to timestamp and extract the year
...
rfr = RandomForestRegressor(featuresCol='features', labelCol='total_vaccinations')

# Fit the model on the training data
rfr_model = rfr.fit(train_data)

pred_result = rfr_model.transform(test_data)
...
print("R2 on test data random forest= ", pred_evaluator.evaluate(pred_result))
